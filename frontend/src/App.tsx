...
const pageCopy: Record<Exclude<NavKey, 'overview' | 'import' | 'datasets' | 'map' | 'processing' | 'results' | 'dronedb'>, { title: string; description: string }> = {
  assistant: {
    title: 'Assistent',
    description: 'Den optionalen KI-Assistenten öffnen, ohne den Arbeitsablauf zu verlassen.',
  },
  settings: {
    title: 'Einstellungen',
    description: 'Frontend-Einstellungen und den Status verbundener Dienste prüfen.',
  },
}
    ...
    <div className="page-stack">
      <section className="hero-panel">
        <div>
          ...
          <h2>Georeferenzierte Luftbilder importieren, prüfen, verarbeiten und veröffentlichen.</h2>
          <p className="hero-copy">
            GeoPhoto Converter bündelt Bild-Qualitätssicherung, Kartenkontext, Verarbeitungsaufträge und Artefakte in einer übersichtlichen Bedienoberfläche.
          </p>
        </div>
...
function PlaceholderPage({ active }: { active: Exclude<NavKey, 'overview' | 'import' | 'datasets' | 'map' | 'processing' | 'results' | 'dronedb'> }) {
  ...
  return (
    <section className="panel empty-state">
      ...
      <span className="status-chip status-chip--neutral">Umsetzung vorgemerkt</span>
    </section>
  )
}
