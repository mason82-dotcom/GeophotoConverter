          ...
          {selectedJob && (
            <div className="job-detail">
              ...
              <div className="job-progress-block">
                ...
                <span>{selectedJob.message || 'Warten auf Backend-Status …'}</span>
              </div>
                ...
                <section className="job-subpanel">
                  ...
                  <pre className="log-tail" tabIndex={0}>
                    {logs?.available && logs.lines.length
                      ? logs.lines.join('\n')
                      : 'Worker-Protokoll wurde noch nicht erzeugt.'}
                  </pre>
                </section>
                      ...
                      selectedJob.artifacts.map((artifact, index) => (
                        <div className="artifact-row" key={`${artifact.name}-${index}`}>
                          ...
                          {artifact.download_url ? (
                            <a className="mini-button" href={artifact.download_url} download title={`Herunterladen: ${artifact.name}`}>
                              <Herunterladen: size={15} />
                              <span className="visually-hidden">Herunterladen: {artifact.name}</span>
                            </a>
                          ) : (
