...
def test_odm_staging_writes_fh2_prioritized_geo_override(
    ...
    assert georef["fusion_conflict_files"] == 2
    assert georef["fusion_conflict_count"] == 6
...
def test_odm_geo_override_never_promotes_relative_or_generic_altitude_to_z(
    monkeypatch,
    tmp_path: Path,
) -> None:
    records = [
        _record(
            ...
            "DJI_0101_D.JPG",
            {
                "gps": {"latitude": 49.1, "longitude": 8.5, "altitude": 150.0},
        ...
        _record(
            ...
            "DJI_0102_D.JPG",
            {
                "gps": {"latitude": 49.2, "longitude": 8.6, "altitude": 151.0},
...
def test_odm_geo_override_falls_back_when_position_coverage_is_incomplete(
    monkeypatch,
    tmp_path: Path,
) -> None:
    records = [
        _record(
            tmp_path,
            "DJI_0201_D.JPG",
            _file_metadata(49.1, 8.5),
        ),
        _record(
            tmp_path,
            "DJI_0202_D.JPG",
            _file_metadata(None, None),
        ),
    ]
    monkeypatch.setattr(worker_images, "_dataset_files", lambda _: records)
...
def test_odm_result_evidence_verifies_only_inspected_georeferencing(
    monkeypatch,
    tmp_path: Path,
) -> None:
    ...
    def fake_run(command, **kwargs):
        if command[0] == "gdalinfo":
            payload = {
                "size": [1024, 768],
                "coordinateSystem": {"wkt": 'PROJCRS["WGS 84 / UTM zone 32N"]'},
                "geoTransform": [500000.0, 0.05, 0.0, 5430000.0, 0.0, -0.05],
                "stac": {"proj:epsg": 32632},
            }
        elif command[0] == "pdal":
            payload = {
                "metadata": {
                    "comp_spatialreference": 'PROJCRS["WGS 84 / UTM zone 32N"]'
                }
            }
        else:
...
def test_odm_result_evidence_does_not_claim_crs_when_inspector_missing(
    monkeypatch,
    tmp_path: Path,
) -> None:
    project = tmp_path / "project"
    path = project / "odm_orthophoto" / "odm_orthophoto.tif"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_bytes(b"artifact")
    ...
    _, evidence = odm_evidence.build_odm_result_evidence(project, {})

    orthophoto = evidence["outputs"]["orthophoto"]
