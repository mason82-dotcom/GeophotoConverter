...
def test_capture_geometry_uses_fh2_canonical_height_and_pose() -> None:
    metadata = {
        "image": {
            "width": 4000,
            "height": 3000,
            "focal_length": 10.0,
            "focal_length_35mm": 20.0,
        },
        "gps": {},
        "dji": {},
    }
    ...
    geometry = estimate_capture_geometry(metadata, fh2_media)

    assert geometry["status"] == "available"
    assert geometry["relative_height_m"] == 100.0
    assert geometry["gimbal_pitch_deg"] == -90.0
