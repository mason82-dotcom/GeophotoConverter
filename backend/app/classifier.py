...
def _dji_metadata(metadata: dict[str, Any] | None) -> dict[str, Any]:
    return (metadata or {}).get("dji") or {}


def _capture_uuid(metadata: dict[str, Any] | None) -> str | None:
    value = _dji_metadata(metadata).get("capture_uuid")
...
def _metadata_band_kind(metadata: dict[str, Any] | None) -> str | None:
    value = _dji_metadata(metadata).get("band_name")
...
def classify_media(
    ...
    if metadata_band:
        ...
        return MediaClassification(
            platform="M3M" if platform == "UNKNOWN" else platform,
