...
def odm_geo_arguments(
    ...
    if (
        ...
        return []
    return ["--geo", str(project_dir / str(georeferencing["path"]))]
...
def _valid_geotransform(value: Any) -> bool:
    ...
    # GDAL affine basis vectors:
    # x pixel vector=(GT1, GT4), y pixel vector=(GT2, GT5).
    return hypot(numbers[1], numbers[4]) > 0.0 and hypot(
        numbers[2], numbers[5]
    ) > 0.0
...
def inspect_geotiff(path: Path) -> dict[str, Any]:
    ...
    has_geotransform = _valid_geotransform(geotransform)
    evidence.update(
        {
            "has_crs": has_crs,
...
def build_odm_result_evidence(
    ...
    evidence = {
        ...
        "interpretation": (
            "Raster georeferencing is verified only when gdalinfo reports both "
            "a CRS and a non-degenerate GeoTransform. Missing inspection tools "
            "or inspection errors never imply valid georeferencing."
        ),
    }
