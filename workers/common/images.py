...
def _canonical_georeferencing(
    ...
    from app.photogrammetry_geo import absolute_georeference_height
    ...
    absolute_height = absolute_georeference_height(canonical)
    return {
        "relative_path": record["relative_path"],
        ...
        "ellipsoid_m": (
            absolute_height["value_m"]
            if absolute_height["status"] == "ready"
            else None
        ),
        "absolute_height_reference": absolute_height.get("reference"),
...
def _write_geo_override(
    ...
    evidence: dict[str, Any] = {
        ...
        "absolute_height_reference": (
            "wgs84_ellipsoidal"
            if entries and len(ellipsoid_entries) == len(entries)
            else None
        ),
