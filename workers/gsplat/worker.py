...
def _verify_colmap_runtime(log_path: Path) -> None:
    ...
    if (
        ...
        or f"COLMAP {COLMAP_VERSION}" not in output
        or "with CUDA" not in output
    ):
        raise RuntimeError(
            f"COLMAP {COLMAP_VERSION} mit CUDA ist im gsplat-Container nicht verfügbar."
        )
...
def handle(payload: dict) -> None:
    ...
    update_job(
        ...
        message=(
            ...
            f"{image_count} RGB/WIDE-Bildern mit CUDA "
    ...
    update_job(
        ...
        message=f"COLMAP {profile['matcher']} gleicht Bildmerkmale mit CUDA ab.",
    )
    if not _run(
        job_id,
        scene_dir,
